...
@Component({
  selector: 'page-searchtournaments',
  templateUrl: 'searchtournaments.html',
})
export class SearchTournamentsPage {
...
public mysearch="";
  ...
  getTournaments(){
    ...
    //this.http.get(this.global.url+"/clubs/2019", {headers}).subscribe((resp:any) => {
	    ...
	    this.http.get(this.global.urlApiLocal+"/getData.php?e=competitions/"+this.global.season+"/"+this.tournamentType.Id+"&s=&l="+this.perPage+"&p="+this.page+"&token="+encodeURIComponent(this.global.token)).subscribe((resp:any) => {
  ...
  searchTournaments(string){
    ...
    //this.http.get(this.global.url+"/clubs/2019", {headers}).subscribe((resp:any) => {
	    //this.http.get(this.global.urlApiLocal+"/getGolfs.php?file=true&token="+encodeURIComponent(this.global.token)).subscribe((resp:any) => {
	    this.http.get(this.global.urlApiLocal+"/getData.php?e=competitions/"+this.global.season+"/"+this.tournamentType.Id+"&s="+string+"&l=205&p=1&token="+encodeURIComponent(this.global.token)).subscribe((resp:any) => {
	    //this.http.get(this.global.url+"/golfs/1/150", {headers}).subscribe((resp:any) => {
	    console.log(resp);
	    for(let i=0; i<resp.length; i++) {
             this.tournaments.push(resp[i]);
